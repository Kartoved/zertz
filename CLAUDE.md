...
- `tournamentStore.ts` — arena tournaments (list + detail polling, create/join/pause/resume, edit/delete, recurring schedules)
...
**Localization** (`src/locales/`) — TypeScript objects, three languages: `ru`, `en`, `eo`. The active language lives in `uiStore` (`localStorage['zertz_language']`). With nothing stored, `src/utils/detectLanguage.ts` guesses: `ru` for Russia/CIS browser languages (`ru`, `be`, `kk`, `ky`, `uz`, `tg`, `tk`, `hy`, `az`, …) with a timezone fallback for en-locale browsers physically in the region, `en` otherwise. **`uk` is deliberately excluded** from both lists. A guess is NOT persisted — only an explicit `setLanguage`/`cycleLanguage` writes the key — so the user's own choice always wins afterwards. Esperanto is never auto-selected. `<html lang>` is kept in sync by `applyHtmlLang`.

**Branding assets live in `public/`, all generated from `logo.jpg` at the repo root**: `favicon.ico` (PNG-in-ICO, 16+32), `favicon-{16,32}x{16,32}.png`, `apple-touch-icon-180x180.png`, `pwa-{192,512}.png` (also the OG/Twitter preview image) and `logo.png` (256px, the main-menu header). Regenerate them all together if the logo changes — the old gradient `icon.svg` is no longer referenced.
...
| `/api/studies` | `routes/studies.js` | Studies (Lichess-style lessons): Notion-like hierarchy CRUD, public list, per-`:owner/:slug` fetch, per-author visible tree (`GET /tree/:owner`, guests included), move/reorder, deep clone |
| `/api/tournaments` | `routes/tournaments.js` | Arena tournaments: create (one-off / recurring `/schedules`), list, detail+standings (`GET /:id`), join/pause/resume, edit/delete, per-tournament games (`GET /:id/games`). See the Arena convention below + `docs/ARENA_TOURNAMENTS.md`. |

`server/db.js` owns the PostgreSQL pool and creates all tables on startup (idempotent). Tables include `position_moves` (per-position move aggregates) and `position_games` (per-game references; see `shared/explorer/`), plus the arena tables `tournaments`, `tournament_players`, `tournament_games`, `tournament_schedules`.
...
`server/arenaEngine.js` — the **arena tournament engine**: a single `setInterval` (~4s) started in `server.js` `startServer()`. **This is the only server-side background timer in the codebase** (everything else is request-driven / read-time). Each tick it materializes recurring schedules, transitions tournament windows (scheduled→active→finished), reconciles finished games into scores, and auto-pairs free players into new rooms. Safe as a single in-process loop only because deployment is single-instance Docker Compose; all state lives in the DB so it survives restarts.
...
- `arena.js` — PURE arena helpers (pairing, scoring, next-occurrence) with unit tests (`arena.test.js`); consumed by `arenaEngine.js`
- `pgTime.js` — `pgUtc(ms)`: format epoch-ms as a bare UTC timestamp string for `timestamp` columns
...
**Studies (Lichess-style lessons) live under `src/components/Studies/` + `src/store/studyStore.ts`.** Route `/studies` and `/studies/:owner/:slug` (`App.tsx`), reached from the **Learning** menu. Data model is one `studies` table (`server/db.js`) where **every row is both content AND a Notion-like nestable node** (`parent_id`, fractional `sort`); `setup_json` is the starting `GameState`, `tree_json` is a `GameNode` tree (with an optional `comment` on nodes), plus `root_comment`/`meta_json`/`is_public`. `slug` is a user-editable, per-owner-unique, transliterated handle (`server/utils/slug.js`). The board interaction **reuses the pure `analysisActions` helpers** (placement/ring-removal/capture) — reader moves branch into a LOCAL working tree and are never persisted; the author's edits **autosave** (debounced, owner-only, skipped mid-`ringRemoval`) via `studyStore.saveStudyTree` → `PUT /:id`. Position reconstruction uses `studyStateAtNode` (replays from `setup_json`, NOT `createInitialState`, then `normalizePhase`) — so a study may start from a custom position. The arbitrary-position editor (`PositionEditorModal`) conserves the marble supply (6/8/10) by construction and uses `HexBoard editable` (removed rings render as clickable ghost slots). `SaveToStudy` (mounted in `RoomScreen`) turns any live/analysis position into a new study. **The chapter sidebar has two modes.** `GET /api/studies/tree` (auth) feeds the author's own editable hierarchy; `GET /api/studies/tree/:owner` (`optionalAuth`, **guests included**) feeds the read-only table of contents shown whenever the URL owner ≠ the signed-in user — it returns only public nodes (everything if the viewer IS the owner) and **lifts a public child of a private parent to the root** so it can't dangle. `StudySidebar` takes `nodes`/`readOnly`/`heading`; read-only hides create/rename/delete/drag. Folding is stored INVERTED as `studyStore.collapsed` (a set of collapsed ids) so new nodes and first-time visitors start expanded, persisted in `localStorage['zertz_study_collapsed']`; a collapse/expand-all button lives in the sidebar header, and the branch to the open chapter auto-expands. **Board annotations** (Lichess-style right-click arrows/circles) live on `GameNode.shapes?: Shape[]` (`{orig,dest?,brush}`; 4 colors via Shift/Alt/Ctrl); `HexBoard`'s `drawable`/`shapes`/`onShapeDraw` props handle rendering + right-drag drawing with a live preview (cursor→SVG coords via `getScreenCTM().inverse()`), studies+desktop only. If you touch the interaction, note it is the SAME code path as online analysis. Full build history: `docs/STUDIES_PLAN.md`.

**Arena tournaments live under `src/components/Tournaments/` + `src/store/tournamentStore.ts` + `server/{routes/tournaments.js, arenaEngine.js, utils/arena.js}`.** Routes `/tournaments` (list+create) and `/tournaments/:id` (detail: standings, live/finished games, owner controls), reached from the top-level **Tournaments** nav. Lichess-style Arena: a time-boxed event; the background engine (`arenaEngine.js`, the codebase's only `setInterval`) continuously auto-pairs free players into rated **blitz 5+5** games, re-pairs on finish, and ranks by points. **Pairing + scoring are PURE functions in `server/utils/arena.js`** (`buildPairings`, `applyGameResult`, `nextOccurrence`) — the only real unit coverage (`arena.test.js`); keep them pure. Scoring: win=2, loss=0, **streak ×2 when streak≥2 ("on fire")**, **berserk +1** (halve your own clock before your first move; `POST /api/rooms/:id/berserk`). Non-obvious gotchas: (1) arena rooms set **`clock_running_since = NOW()` at creation** so clocks tick immediately — an AFK paired player flags without moving and the engine auto-pauses them. (2) **Recurring** tournaments (`tournament_schedules` + `schedule_id`/`reminder_sent` on `tournaments`) store recurrence as a **UTC weekday+minute-of-day derived from the creator's first-run instant** (DST-free); `materializeSchedules()` keeps one upcoming instance and `last_start_at` makes deleting an instance SKIP that occurrence. (3) **Resigning works on any turn** — `PUT /api/rooms/:id/state` lets `winType==='surrender'` bypass the turn check + clock deduction. (4) **Browse-while-playing**: `/tournaments/:id` auto-opens your game ONLY on a NEW pairing (tracked in `localStorage['zertz_arena_entered:<tid>']`); RoomScreen has a header "back to arena" link during live play. (5) **Games on the page**: `GET /api/tournaments/:id/games` → `{live:[…w/ stateJson], finished:[…result only]}`; live render as mini boards (`TournamentLiveGames.tsx`, `deserializeState`+`<HexBoard state preview/>`, click→`/room/:id?watch=1`), finished as clickable result markers inside each standings row + a desktop lazy hover-preview. Full design: `docs/ARENA_TOURNAMENTS.md`.
