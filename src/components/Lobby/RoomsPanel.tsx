...
import { getPendingRooms, getOpenRooms, deleteRoom, PendingRoom } from '../../db/roomsApi';
...
function RoomMeta({ room, t }: { room: PendingRoom; t: any }) {
  return (
    <div className="flex items-center gap-2 flex-wrap">
      <span className="text-xs px-1.5 py-0.5 rounded bg-gray-100 dark:bg-gray-700 font-mono text-gray-600 dark:text-gray-300">{room.boardSize}</span>
...
export default function RoomsPanel({ onCreateGame, currentGames, onLoadGame }: RoomsPanelProps) {
  ...
  const [myRooms, setMyRooms] = useState<PendingRoom[]>([]);
  const [openRooms, setOpenRooms] = useState<PendingRoom[]>([]);
  const [copiedRoomId, setCopiedRoomId] = useState<number | null>(null);

  const fetchRooms = useCallback(async () => {
    // Always fetch public open rooms (guests too)
    try {
      const all = await getOpenRooms();
      setOpenRooms(all);
    } catch { /* ignore */ }

    // My rooms only when authenticated
    if (user) {
      try {
        const mine = await getPendingRooms();
        setMyRooms(mine);
      } catch { /* ignore */ }
    } else {
      setMyRooms([]);
  ...
  const handleCancel = useCallback(async (roomId: number) => {
    try {
      ...
      setMyRooms(rs => rs.filter(r => r.id !== roomId));
      setOpenRooms(rs => rs.filter(r => r.id !== roomId));
      await deleteGame(String(roomId)).catch(() => {});
  ...
  const ArrowIcon = () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"/></svg>;
          ...
          {myRooms.map(room => (
            <div key={room.id} className="flex items-center gap-3 px-4 py-3 rounded-xl bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-200 dark:border-indigo-700">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-indigo-500 dark:text-indigo-400 mt-1">{t.lobbyWaiting}</p>
              </div>
              <div className="flex items-center gap-1 flex-shrink-0">
                <button onClick={() => handleCopy(room.id)} title={t.copyLink}
                  ...
                  {copiedRoomId === room.id ? <CheckIcon /> : <CopyIcon />}
                </button>
                <button onClick={() => navigate(`/room/${room.id}`)} title={t.goToGame}
                  className="w-8 h-8 flex items-center justify-center rounded-lg bg-white dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 text-gray-600 dark:text-gray-300 transition-colors border border-gray-200 dark:border-gray-600">
                  <ArrowIcon />
                </button>
                <button onClick={() => handleCancel(room.id)} title={t.lobbyCancel}
