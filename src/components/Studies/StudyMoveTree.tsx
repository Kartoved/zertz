...
function renderLine(node: GameNode, depth: number, currentId: string, onSelect: (n: GameNode) => void): ReactElement[] {
  const rows: ReactElement[] = [];
  let cur: GameNode | null = node;
  while (cur && cur.move) {
    const here: GameNode = cur;
    rows.push(
      <div key={here.id} style={{ paddingLeft: depth * 14 }}>
        <MoveChip node={here} currentId={currentId} onSelect={onSelect} />
      </div>
    );
    const kids = here.children.filter(k => k.move);
    for (let i = 1; i < kids.length; i++) {
      rows.push(<div key={`${kids[i].id}-var`}>{renderLine(kids[i], depth + 1, currentId, onSelect)}</div>);
    }
    cur = kids[0] ?? null;
  }
  return rows;
...
export default function StudyMoveTree({ root, currentId, onSelect }: StudyMoveTreeProps) {
