...
export default function MarbleSelector(props: MarbleSelectorProps = {}) {
  ...
  const marbles: { color: MarbleColor; count: number }[] = [
    { color: 'white', count: sourceCounts.white },
    { color: 'gray', count: sourceCounts.gray },
    { color: 'black', count: sourceCounts.black },
  ];
      ...
      <div className="flex gap-2 justify-between">
        {marbles.map(({ color, count }) => (
          <button
            ...
            className={`
              flex flex-col flex-1 items-center gap-1.5 p-2 rounded-xl transition-all
          ...
          >
            <div
              className={`
                w-10 h-10 rounded-full border-[3px] shadow-sm
