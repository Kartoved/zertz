export const ru = {
  appSubtitle: 'Абстрактная стратегическая игра',
  developedBy: 'Developed by Wood Romanov',
  profile: 'Профиль',
  loginRegister: 'Войти / Зарегистрироваться',
  playLocal: 'Играть локально',
  loadGame: 'Просмотр партий',
  rules: 'Правила',
  playOnline: 'Сыграть онлайн',
  players: 'Игроки',
  challenges: 'Вызовы',
  tasks: 'Задачи',
  community: 'Комьюнити',
  blog: 'Новости',
  blogEmpty: 'Пока нет новостей',
  blogPostNotFound: 'Новость не найдена',
  blogNewPost: 'Новая статья',
  loadCurrent: 'Текущие',
  loadCompleted: 'Завершённые',
  filterAll: 'Все',
  filterLocal: 'Локальные',
  filterOnline: 'Онлайн',
  noGames: 'Нет игр',
  localLabel: 'локальная',
  onlineLabel: 'онлайн',
  moves: 'Ходов',
  board: 'Поле',
  inProgress: 'В процессе',
  board37: 'Любительское 37 колец',
  board48: 'Турнирное 48 колец',
  board61: 'Турнирное 61 кольцо',
  selectBoard: 'Выбери размер поля',
  chooseBoard: 'Выбери поле',
  chooseBoardOnline: 'Выбери размер поля',
  choosePlayer: 'Выбери игрока',
  inviteLinkTitle: 'Ссылка для приглашения',
  onlineBoardPrompt: 'Выбери размер поля для онлайн-игры.',
  playerPrompt: 'Каким игроком ты хочешь играть?',
  playerFirst: 'Первый игрок',
  playerFirstHint: 'Ходи первым',
  playerSecond: 'Второй игрок',
  playerSecondHint: 'Ходи вторым',
  playerRandom: 'Случайно',
  playerRandomHint: 'Случайный выбор',
  ratedGame: 'Рейтинговая игра',
  ratedStats: 'Рейтинговая статистика',
  ratedHint: 'Влияет на рейтинг Глико',
  back: 'Назад',
  createGame: 'Создать игру',
  creating: 'Создание...',
  createRoomError: 'Не удалось создать комнату. Проверьте подключение к серверу.',
  gameCreatedHint: 'Игра создана! Отправьте ссылку другу для подключения.',
  gameLink: 'Ссылка на игру:',
  copyLink: 'Копировать ссылку',
  copied: '✓ Скопировано!',
  copyPosition: 'Копировать позицию',
  exportGame: 'Экспорт партии',
  importPosition: 'Импорт позиции',
  importPositionHint: 'Вставьте ZIP-позицию или ZEN-партию',
  importLoad: 'Загрузить',
  importError: 'Не удалось разобрать нотацию',
  goToGame: 'Перейти к игре',
  login: 'Войти',
  register: 'Зарегистрироваться',
  emailLabel: 'Email',
  emailPlaceholder: 'your@email.com',
  emailOptionalHint: 'Необязательно — для входа по ссылке и восстановления доступа',
  loginWithMagicLink: 'Войти по email',
  sendMagicLink: 'Отправить ссылку',
  magicLinkSent: 'Проверьте почту — ссылка действует 1 час',
  magicLinkVerifying: 'Проверяем...',
  magicLinkSuccess: 'Вход выполнен! Перенаправляем...',
  magicLinkError: 'Ссылка недействительна или устарела',
  magicLinkEmailNotFound: 'Аккаунт с таким email не найден',
  orSeparator: 'или',
  emailSavedHint: 'Email сохранён. Теперь можно входить по магической ссылке.',
  removeEmail: 'Удалить email',
  nickname: 'Ник',
  yourNickname: 'Твой ник',
  password: 'Пароль',
  passwordHint: 'Минимум 8 символов, включая спецсимвол',
  confirmPassword: 'Подтверди пароль',
  repeatPassword: 'Повтори пароль',
  fillAllFields: 'Заполни все поля',
  nicknameRangeError: 'Ник должен быть от 2 до 24 символов',
  passwordLengthError: 'Пароль должен быть не менее 8 символов',
  passwordCharError: 'Пароль должен содержать хотя бы один спецсимвол (!@#$%^&*...)',
  passwordsMismatch: 'Пароли не совпадают',
  loading: 'Загрузка...',
  profileUpdated: 'Профиль обновлён',
  profileTitle: 'Профиль',
  registrationDate: 'Регистрация',
  rating: 'Рейтинг',
  games: 'Игр',
  winsLosses: 'Побед / Поражений',
  winrate: 'Винрейт',
  bestStreak: 'Лучшая серия побед',
  quote: 'Цитата',
  quotePlaceholder: 'Твоя цитата или девиз...',
  quoteNotSet: 'Цитата не задана',
  contact: 'Для связи',
  contactPlaceholder: 'https://t.me/username',
  contactInvalid: 'Поле "Для связи" должно содержать корректную ссылку (https://...)',
  country: 'Страна',
  chooseCountry: 'Выбери страну',
  saveProfile: 'Сохранить профиль',
  hidePasswordChange: '▾ Скрыть смену пароля',
  showPasswordChange: '▸ Сменить пароль',
  currentPassword: 'Текущий пароль',
  newPassword: 'Новый пароль',
  newPasswordPlaceholder: 'Новый пароль (мин. 8, со спецсимволом)',
  confirmNewPassword: 'Подтверди новый пароль',
  showPasswords: 'Показать пароли',
  changePassword: 'Сменить пароль',
  fillAllPasswordFields: 'Заполни все поля пароля',
  newPasswordLengthError: 'Новый пароль должен быть не менее 8 символов',
  newPasswordCharError: 'Новый пароль должен содержать хотя бы один спецсимвол',
  newPasswordsMismatch: 'Новые пароли не совпадают',
  passwordChanged: 'Пароль изменён',
  logout: 'Выйти из аккаунта',
  allPlayers: 'Все игроки',
  friends: 'Друзья',
  searchByNickname: 'Поиск по нику...',
  noPlayers: 'Нет игроков',
  noFriends: 'Нет подписок',
  player: 'Игрок',
  winsShort: 'Побед',
  lossesShort: 'Пораж.',
  regShort: 'Рег.',
  unfollowed: 'Отписка оформлена',
  followed: 'Подписка оформлена',
  incoming: 'Входящие',
  incomingChallenges: 'Входящие вызовы',
  outgoing: 'Исходящие',
  noIncoming: 'Нет входящих вызовов',
  noOutgoing: 'Нет исходящих вызовов',
  challengeDeclined: 'Вызов отклонён',
  challengeCanceled: 'Вызов отменён',
  accepted: 'Принять',
  decline: 'Отклонить',
  cancel: 'Отменить',
  boardLabel: 'Поле',
  rated: 'Рейтинговая',
  roomLoadingError: 'Ошибка загрузки профиля',
  challengeSent: 'Вызов отправлен!',
  playerNotFound: 'Игрок не найден',
  close: 'Закрыть',
  registered: 'Зарегистрирован',
  follow: 'Подписаться',
  unfollow: 'Отписаться',
  challengeToGame: 'Вызвать на игру',
  challengeSettings: 'Настройки вызова',
  firstShort: '1-й',
  secondShort: '2-й',
  random: 'Случайно',
  sendChallenge: 'Отправить вызов',
  sending: 'Отправка...',
  menu: 'Меню',
  undo: 'Отменить',
  capturedMarbles: 'Захваченные шарики:',
  move: 'Ход',
  removeRing: 'Удали кольцо',
  mandatoryCapture: 'Обязательное взятие!',
  chooseMarble: 'Выбери шарик:',
  noMovesYet: 'Ходов пока нет',
  moveStart: 'старт',
  deleteMovesConfirm: 'Удалить ходы, начиная с этого?',
  roomMenu: 'Меню',
  roomRules: 'Правила',
  copyRoomLink: 'Скопировать ссылку',
  you: 'Ты',
  yourTurn: 'Твой ход',
  opponentTurn: 'Ход соперника',
  chooseMarbleShort: 'Выбери шарик:',
  undoMove: 'Вернуть ход назад',
  expandChat: 'Развернуть чат',
  collapseChat: 'Свернуть чат',
  chat: 'Чат',
  noMessagesYet: 'Пока нет сообщений',
  loadEarlierMessages: 'Загрузить ранние сообщения',
  messagePlaceholder: 'Сообщение...',
  gameOver: 'Партия завершена',
  gameRulesTitle: 'Правила игры ZERTZ',
  rematch: 'Реванш',
  backToMenu: 'В меню',
  rematchChooseBoard: 'Реванш: выбери поле',
  winByWhite: 'Победа по белым шарикам!',
  winByGray: 'Победа по серым шарикам!',
  winByBlack: 'Победа по чёрным шарикам!',
  winByMixed: 'Победа по разным шарикам!',
  winByTime: 'Победа по времени!',
  winUnknown: 'Победа!',
  phasePlacement: 'Размести шарик',
  phaseRingRemoval: 'Удали кольцо',
  phaseCapture: 'Захвати шарик',
  goalTitle: 'Цель игры',
  goalLead: 'Первым захватить:',
  goal1: '4 белых шарика, ИЛИ',
  goal2: '5 серых шариков, ИЛИ',
  goal3: '6 чёрных шариков, ИЛИ',
  goal4: '3 шарика каждого цвета',
  flowTitle: 'Ход игры',
  flowLead: 'На каждом ходу игрок выполняет ОДНО из двух действий:',
  flowPlacementTitle: '1. Размещение + удаление кольца',
  flowPlace1: 'Выбери шарик любого цвета из резерва',
  flowPlace2: 'Поставь его на любое пустое кольцо',
  flowPlace3: 'Удали одно «свободное» кольцо с края доски, если это возможно',
  flowCaptureTitle: '2. Взятие (обязательно!)',
  flowCaptureLead: 'Если можешь взять — ОБЯЗАН взять!',
  flowCapture1: 'Прыгните через соседний шарик на пустое кольцо за ним',
  flowCapture2: 'Цвет не имеет значения',
  flowCapture3: 'Цепочки взятий - продолжайте захватывать шарики пока это возможно',
  reserveTitle: 'Резерв шариков',
  reserveText1: 'В начале игры в',
  reserveText2: 'общем резерве',
  reserveCounts: '10 чёрных, 8 серых и 6 белых.',
  reserveSectionTitle: 'Резерв',
  reserveSectionText1: 'Резерв - это место, где хранятся шарики, которые еще не были использованы в игре.',
  reserveSectionText2: 'Игроки могут брать шарики из резерва и использовать их в своей игре.',
  freeRingTitle: 'Свободное кольцо',
  freeRingLead: 'Кольцо можно удалить если:',
  freeRing1: 'На нём нет шарика',
  freeRing2: 'У него ≥2 свободных соседних стороны',
  isolationTitle: 'Изоляция',
  isolationText1: 'Если группа колец отделяется от основной доски и все кольца заняты —',
  isolationText2: 'все шарики в группе автоматически захватываются текущим игроком!',
  blitz: 'Блиц',
  rapid: 'Рапид',
  long: 'Классика',
  correspondence: 'Асинхронная',
  playByLink: 'Сыграть по ссылке',
  comingSoon: 'Скоро',
  globalChat: 'Общий чат',
  guest: 'Гость',
  settings: 'Настройки',
  language: 'Язык',
  playCorrespondence: 'Играть асинхронно',
  selectTimeControl: 'Выбери контроль времени',
  darkMode: 'Тёмная тема',
  lightMode: 'Светлая тема',
  openMenu: 'Открыть меню',
  closeMenu: 'Закрыть меню',
  chatLoginToWrite: 'Войди, чтобы писать в чат',
  send: 'Отправить',
  zertzByKrisBurm: 'Zertz by Kris Burm',
  tabPlay: 'Игра',
  tabPlan: 'План',
  tabBoard: 'Доска',
  tabControls: 'Панель',
  tabPlayers: 'Игроки',
  surrender: 'Сдаться',
  addTime: 'Добавить время сопернику',
  addTimeBlitz: '+15с',
  addTimeCorr: '+1 день',
  confirmSurrenderTitle: 'Подтверди сдачу',
  confirmSurrenderText: 'Ты уверен, что хочешь сдаться? Это приведет к немедленному поражению.',
  confirmAction: 'Да, сдаться',
  cancelAction: 'Отмена',
  undoRequestChat: 'Просит отменить ход',
  undoAccept: 'Разрешить',
  undoReject: 'Отказ',
  undoRejectedChat: 'Запрос на отмену хода отклонён.',
  undoAcceptedChat: 'Запрос на отмену хода принят.',
  tabMainPlay: 'Игра',
  tabMainLearning: 'Обучение',
  tabMainCommunity: 'Комьюнити',
  tabMainNews: 'Новости',
  whatsNew: 'Что нового?',
  onlineRequiresAuth: 'Для онлайн-игры необходима регистрация',
  cancelGame: 'Аннулировать',
  gameCancelled: 'Партия аннулирована',
  cancelledStatus: 'Аннулирована',
  viewGamesBtn: 'Партии',
  gamesBrowser: 'Партии',
  tabActive: 'Активные',
  tabFinished: 'Завершённые',
  filter: 'Фильтр',
  youAreSpectator: 'Вы наблюдаете',
  joinSeatTitle: 'Есть свободное место',
  joinSeatPrompt: 'Хотите сыграть или наблюдать?',
  joinAsPlayer: 'Сыграть',
  archiveAllGames: 'Архив всех партий',
  watchGame: 'Смотреть',
  loadArchive: 'Архив',
  pushNotifications: 'Уведомления',
  pushEnable: 'Включить уведомления',
  pushDisable: 'Выключить уведомления',
  pushNotSupported: 'Не поддерживается',
  botGameSetup: 'Игра против бота',
  botDifficulty: 'Сложность',
  botEasy: 'Лёгкий',
  botMedium: 'Средний',
  botHard: 'Сложный',
  botSide: 'Вы играете за',
  botSidePlayer1: 'Первого (ходите первым)',
  botSidePlayer2: 'Второго (бот ходит первым)',
  botThinking: 'Бот думает...',
  startBotGame: 'Начать игру',
  timeControl: 'Контроль',
  loginToPlay: 'Войдите, чтобы создавать и принимать игры',
  openRooms: 'Открытые комнаты',
  lobby: 'Лобби',
  lobbyEmpty: 'Нет открытых комнат — создай первым!',
  lobbyCreate: 'Создать игру',
  lobbyCancel: 'Отменить',
  lobbyJoin: 'Принять',
  lobbyWaiting: 'Ожидаем соперника...',
  lobbyRated: 'Рейтинговая',
  lobbyUnrated: 'Нерейтинговая',
  iteraciaClub: 'Клуб Итерация',
  vkCommunity: 'Сообщество ВК',
  discordCommunity: 'Дискорд сообщество',
  nicknameTaken: 'Ник уже занят',
  emailAlreadyTaken: 'Этот email уже используется',
  invalidCredentials: 'Неверный ник или пароль',
  wrongCurrentPassword: 'Неверный текущий пароль',
  serverError: 'Ошибка сервера',
  analysis: 'Анализ',
  analysisMode: 'Режим анализа',
  returnToGame: 'Вернуться к игре',
  liveMoveDuringAnalysis: 'В партии сыгран ход',
  exitAnalysis: 'Выйти из анализа',
  openingExplorer: 'Дебютный обозреватель',
  explorerOnlyMine: 'Только мои',
  explorerFilterMineHint: 'Показать только партии с моим участием',
  explorerNoGames: 'Из этой позиции пока никто не играл',
  explorerTotalGames: 'Всего партий',
  explorerLoadError: 'Не удалось загрузить данные',
  conditionalPremoves: 'Условные предходы',
  deleteVariant: 'Удалить',
  premovesHint: 'Если соперник сходит так, как ты предположил — твой ответ выполнится автоматически.',
  clearAllVariants: 'Очистить все',
  confirmPlayOwnMove: 'Первый ход варианта — ваш ({move}). Сыграть его сейчас на доске? Остальное сохранится как предходы.',
  premoveFiredToast: 'Ваш предход сыгран автоматически: {move}',
  premovePrunedToast: 'Предходы сброшены: партия отклонилась от плана',
  premoveArmed: 'Предход заряжен',
  confirmSendMove: 'Отправить ход {move}?',
  sendMove: 'Отправить',
  armFromAnalysis: 'Зарядить план из анализа',
  playAndArm: 'Сыграть и зарядить',
  planBuildHint: 'Постройте варианты на доске — они появятся здесь как дерево. Клик по ходу — перейти к позиции, ✕ — удалить ветку.',
  premoveArmedCount: 'Заряжено веток: {n}',
  armFromAnalysisHint: 'Построй на доске варианты ответов соперника и свои ответы (ветки появятся в журнале ходов), затем нажми — зарядятся все ветки разом.',
  armNothingToArm: 'Нечего заряжать: постройте в анализе ходы соперника и свои ответы на них.',
  armIncompleteMove: 'Ваш ход не завершён: после постановки шарика нужно снять кольцо. Доиграйте ход в анализе (уберите кольцо), затем заряжайте.',
  armDroppedAlternatives: 'План заряжен. Для некоторых позиций у вас было несколько своих ходов — взят основной (первый). Из одной позиции можно зарядить только один свой ход.',
  statusOnline: 'Онлайн',
  statusOffline: 'Не в сети',
  onlinePlayers: 'Игроки онлайн',
  noOneOnline: 'Сейчас никого нет',
  zertzTv: 'ZERTZ TV',
  tvLastGame: 'Последняя партия',
  tvNoGames: 'Пока нет партий',
  studies: 'Студии',
  myStudies: 'Мои студии',
  studyNew: 'Новая',
  studyNewChild: 'Вложенная студия',
  studyRename: 'Переименовать',
  studyDelete: 'Удалить',
  studyPublic: 'Публичная',
  studyEmpty: 'Пусто',
  studyNewTitlePrompt: 'Название студии:',
  studyRenamePrompt: 'Новое название:',
  studyDeleteConfirm: 'Удалить студию?',
  studyDeleteWithChildren: 'Удалить студию со всеми вложенными?',
  studyExploreHint: 'Играйте варианты на доске — это ваше исследование, оно не сохраняется.',
  studyEditHint: 'Играйте ходы на доске — это ваша студия. Изменения сохраняются автоматически.',
  studyResetLine: 'К линии автора',
  studyStart: 'Начало',
  studySaving: 'Сохранение…',
  studySaved: 'Сохранено',
  studyDeleteVariantConfirm: 'Удалить эту ветку?',
  studySave: 'Сохранить',
  studyCommentPlaceholder: 'Комментарий к позиции (markdown)…',
  studyTabEdit: 'Редактор',
  studyTabRead: 'Чтение',
  studyNoComment: 'Нет комментария к этой позиции',
  studyDrawHint: 'ПКМ — стрелки и маркеры (Shift/Alt/Ctrl — цвет)',
  studyTraining: 'Тренировка',
  studyTrainingHidden: 'Решение скрыто',
  metaTraining: 'Открывать в режиме тренировки (решение скрыто)',
  studyClearShapes: 'Очистить пометки',
  saveToStudy: 'В студию',
  saveToStudyParent: 'Куда сохранить',
  saveToStudyRoot: 'Новая студия (корень)',
  studyMeta: 'Мета-инфо',
  metaEvent: 'Турнир',
  metaRound: 'Тур',
  metaTimeControl: 'Контроль',
  metaDate: 'Дата',
  metaResult: 'Результат',
  studyPlayer1: 'Игрок 1',
  studyPlayer2: 'Игрок 2',
  studyNewFromPosition: 'Из позиции',
  studyImport: 'Импорт ZEN/ZIP',
  studiesAll: 'Все студии',
  studiesBack: '← К студиям',
  studyImportTitle: 'Название (необязательно)',
  editorTitle: 'Редактор позиции',
  editorEraseMarble: 'Убрать шарик',
  editorToggleRing: 'Убрать/вернуть кольцо',
  editorBoardSize: 'Доска',
  editorTurn: 'Чей ход',
  editorReserve: 'Резерв',
  editorCaptures: 'Взятые',
  editorCreate: 'Создать студию',
  studyClone: 'Скопировать себе',
  studyMakePublic: 'Опубликовать',
  studyMakePrivate: 'Сделать приватной',
  studySlugPrompt: 'Ссылка (slug):',
  studyBy: 'Автор:',
  publicStudies: 'Публичные студии',
  studyWelcome: 'Интерактивные уроки: создавайте разборы с ветками и комментариями.',
  studyNotFound: 'Студия не найдена',
  studyChildren: 'Вложенные студии',
  loginToCreateStudies: 'Войдите, чтобы создавать студии',
  playersTotal: 'Всего',
  playersOnline: 'Онлайн',

  // Arena tournaments
  tournaments: 'Турниры',
  arenaCreate: 'Создать арену',
  arenaName: 'Название',
  arenaStartsInMin: 'Старт через (мин)',
  arenaDurationMin: 'Длительность (мин)',
  arenaBerserk: 'Берсерк',
  arenaSectionActive: 'Идут сейчас',
  arenaSectionUpcoming: 'Предстоящие',
  arenaSectionFinished: 'Завершённые',
  arenaStatusScheduled: 'Скоро',
  arenaStatusActive: 'Идёт',
  arenaStatusFinished: 'Завершён',
  arenaJoin: 'Участвовать',
  arenaPause: 'Пауза',
  arenaResume: 'Продолжить',
  arenaStartsIn: 'Старт через',
  arenaTimeLeft: 'Осталось',
  arenaScore: 'Очки',
  arenaStreak: 'Серия',
  arenaGamesCol: 'Игры',
  arenaPlayers: 'Игроки',
  arenaEmpty: 'Пока нет турниров',
  arenaNoPlayers: 'Пока никто не присоединился',
  arenaWinner: 'Победитель',
  arenaGameReady: 'Ваша игра готова',
  arenaGoToGame: 'К игре',
  arenaBackToArena: 'К турниру',
  arenaBerserkBtn: 'Берсерк!',
  arenaBerserkHint: 'Урезать часы вдвое ради +1 очка за победу',
  arenaRatedBlitz: 'Рейтинг · блиц 5+5',
  arenaStartAt: 'Старт (дата и время)',
  arenaRepeat: 'Повтор',
  arenaRepeatOnce: 'Не повторять',
  arenaRepeatDaily: 'Ежедневно',
  arenaRepeatWeekly: 'Еженедельно',
  arenaRecurring: 'Повторяющийся',
  arenaEdit: 'Изменить',
  arenaDelete: 'Удалить',
  arenaSave: 'Сохранить',
  arenaSeries: 'Серия',
  arenaEditSeries: 'Изменить серию',
  arenaSkipOccurrence: 'Пропустить эту дату',
  arenaStopSeries: 'Остановить серию',
  arenaDeleteConfirm: 'Удалить турнир?',
  arenaSkipConfirm: 'Пропустить это проведение? Следующее создастся по расписанию.',
  arenaStopSeriesConfirm: 'Остановить серию? Будущие турниры больше не будут создаваться.',
  arenaResults: 'Итоги',
  arenaLiveGames: 'Идут сейчас',
  arenaWatch: 'Смотреть',
};
